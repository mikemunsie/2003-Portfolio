...
const musicSource = musicElement.querySelector("source");

const downElement = document.getElementById("soundDown");
const downElementSource = downElement.querySelector("source");

const upElement = document.getElementById("soundUp");
const upElementSource = upElement.querySelector("source");

const moveElement = document.getElementById("moveFrame");
const moveElementSource = moveElement.querySelector("source");
...
function move_framef() {
  if (frame_x == 0) {
    musicSource.src = songs[frame_page].src;
    musicElement.load();
    musicElement.play();
    frame_running = false;
    document.all.currentpage.innerHTML =
      "<font style='font-size:10px' face='verdana'>" +
      titles[frame_page] +
      "</font>";
...
function move_frameb() {
  if (frame_x <= -700) {
    ...
    musicElement.play();
    document.all.currentpage.innerHTML =
      "<font style='font-size:10px' face='verdana'>Loading...</font>";
...
function movediv() {
  ...
  if (page_y <= -100) {
    document.all.mydiv.innerHTML =
      "<img onClick='change_page(" +
      page_counter +
      ",25)' border='0' style='cursor:hand' src='../linkbar/" +
      page_counter +
      ".gif'>";
    movedivb();
  } else {
...
function tdeffect() {
  ...
  if (z >= 1) {
    ...
    document.all.tde.style.opacity = z;
  }
}
timer2 = setInterval(tdeffect, 100);
